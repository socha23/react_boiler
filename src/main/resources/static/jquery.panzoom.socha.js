...
(function(global, factory) {
	...
	Panzoom.prototype = {
		...
		setMatrix: function(matrix, options) {
			...
			// Apply containment
            // special socha containment
            if (contain === 'socha') {
                var width = this.elem.getBoundingClientRect().width;
                var height = this.elem.getBoundingClientRect().height;

                var conWidth = this.parent.getBoundingClientRect().width;
                var conHeight = this.parent.getBoundingClientRect().height;

                var zoom = matrix[0];

                let lowerW = (-width / 2) - (width / zoom / 2) + conWidth;
                let upperW = (width / 2) - (width / zoom / 2);
                matrix[4] = Math.min(Math.max(matrix[4], lowerW), upperW);

                let lowerH = (-height / 2) - (height / zoom / 2) + conHeight;
                let upperH = (height / 2) - (height / zoom / 2);
                matrix[5] = Math.min(Math.max(matrix[5], lowerH), upperH);
            } else if (contain) {
